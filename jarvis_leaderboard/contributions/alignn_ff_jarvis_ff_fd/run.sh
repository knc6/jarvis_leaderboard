#!/bin/bash
# Reproduce the CHIPS-FF (dft_3d_chipsff) leaderboard entries for ALIGNN-FF JARVIS-FF-FD.
#
...
: "${MODEL_PATH:?Set MODEL_PATH to the ALIGNN-FF JARVIS-FF-FD ALIGNN-FF model dir (best_model.pt + config.json)}"
# 1) install
...
echo "Model=ALIGNN-FF JARVIS-FF-FD (tag jarvis_ff_fd). See CHIPS-FF: https://github.com/atomgptlab/chipsff"
