#!/bin/bash
# Reproduce the CHIPS-FF (dft_3d_chipsff) leaderboard entries for ALIGNN-FF JARVIS-FF-DB.
#
...
: "${MODEL_PATH:?Set MODEL_PATH to the ALIGNN-FF JARVIS-FF-DB ALIGNN-FF model dir (best_model.pt + config.json)}"
# 1) install
...
echo "Model=ALIGNN-FF JARVIS-FF-DB (tag jarvis_ff_db). See CHIPS-FF: https://github.com/atomgptlab/chipsff"
