#!/bin/bash
# Reproduce AI-SinglePropertyPrediction-Tc-alex_supercon-test-mae.csv
#   ALIGNN 2.0 (kNN graph, cutoff 8.0) on alex_supercon property 'Tc'.
# Config is saved next to this script as config_alex_supercon_Tc.json. Test split is the
# JARVIS-Leaderboard benchmark alex_supercon_Tc.
set -e
...
WORK="$HERE/_work_alex_supercon_Tc"; mkdir -p "$WORK"
cp "$HERE/config_alex_supercon_Tc.json" "$WORK/config.json"
...
name="AI-SinglePropertyPrediction-Tc-alex_supercon-test-mae.csv"
p[["id","prediction"]].to_csv("$HERE/"+name, index=False)
with zipfile.ZipFile("$HERE/AI-SinglePropertyPrediction-Tc-alex_supercon-test-mae.csv.zip","w",zipfile.ZIP_DEFLATED) as z: z.write("$HERE/"+name, name)
os.remove("$HERE/"+name); print("wrote AI-SinglePropertyPrediction-Tc-alex_supercon-test-mae.csv.zip")
PYEOF
