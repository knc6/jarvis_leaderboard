#!/bin/bash
# Reproduce AI-SinglePropertyPrediction-la-alex_supercon-test-mae.csv
#   ALIGNN 2.0 (kNN graph, cutoff 8.0) on alex_supercon property 'la'.
# Config is saved next to this script as config_alex_supercon_la.json. Test split is the
# JARVIS-Leaderboard benchmark alex_supercon_la.
set -e
...
WORK="$HERE/_work_alex_supercon_la"; mkdir -p "$WORK"
cp "$HERE/config_alex_supercon_la.json" "$WORK/config.json"
...
name="AI-SinglePropertyPrediction-la-alex_supercon-test-mae.csv"
p[["id","prediction"]].to_csv("$HERE/"+name, index=False)
with zipfile.ZipFile("$HERE/AI-SinglePropertyPrediction-la-alex_supercon-test-mae.csv.zip","w",zipfile.ZIP_DEFLATED) as z: z.write("$HERE/"+name, name)
os.remove("$HERE/"+name); print("wrote AI-SinglePropertyPrediction-la-alex_supercon-test-mae.csv.zip")
PYEOF
